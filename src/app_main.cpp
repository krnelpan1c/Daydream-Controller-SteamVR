...
int GetHandRole() {
  return GetPrivateProfileIntA("Settings", "HandRole", 1,
                               GetSettingsPath().c_str());
}

void SetHandRole(int role) {
  std::string val = std::to_string(role);
  WritePrivateProfileStringA("Settings", "HandRole", val.c_str(),
                             GetSettingsPath().c_str());
}
...
void SendDataToPipe(const DaydreamData &data) {
  if (g_pipeConnected && g_hPipe != INVALID_HANDLE_VALUE) {
    DWORD written = 0;
    BOOL result =
        WriteFile(g_hPipe, &data, sizeof(DaydreamData), &written, NULL);
    if (!result) {
...
void StartNamedPipeServer() {
  std::thread([]() {
    g_hPipe = CreateNamedPipeA(
        "\\\\.\\pipe\\DaydreamSteamVR", PIPE_ACCESS_OUTBOUND,
        PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 1,
        sizeof(DaydreamData), sizeof(DaydreamData), 0, NULL);
...
void InstallDriver(HWND hwnd) {
  ...
  if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\WOW6432Node\\Valve\\Steam",
    ...
    DWORD size = MAX_PATH;
    if (RegQueryValueExA(hKey, "InstallPath", NULL, NULL, (LPBYTE)steamPath,
                         &size) == ERROR_SUCCESS) {
      std::string vrpathreg =
          std::string(steamPath) +
          "\\steamapps\\common\\SteamVR\\bin\\win64\\vrpathreg.exe";
...
void StartBluetoothHandler(HWND hwnd, uint64_t addr = 0,
                           std::wstring deviceId = L"") {
  g_bleHandler.Start([](const DaydreamData &data) { SendDataToPipe(data); },
                     addr, deviceId);
}

void ScheduleMinimize(HWND hwnd) {
  std::thread([hwnd]() {
    for (int i = 5; i > 0; --i) {
      std::string msg =
          "Controller connected, please launch SteamVR. Minimizing in " +
          std::to_string(i) + " seconds...";
      UpdateStatus(hwnd, msg.c_str());
      Sleep(1000);
    }
    UpdateStatus(hwnd, "Controller connected, please launch SteamVR.");
...
void ConnectController(HWND hwnd) {
  ...
  UpdateStatus(
      hwnd,
      "Searching for Daydream controller... (Make sure white LED is pulsing)");
  ...
  std::thread([hwnd]() {
    try {
      ...
      winrt::hstring selector = L"System.Devices.Aep.ProtocolId:=\"{bb7bb05e-"
                                L"5972-42b5-94fc-76eaa7084d49}\" AND "
                                L"System.Devices.Aep.IsPaired:=System."
                                L"StructuredQueryType.Boolean#True";
      auto devicesInfo = DeviceInformation::FindAllAsync(
                             selector, {L"System.Devices.Aep.DeviceAddress"},
                             DeviceInformationKind::AssociationEndpoint)
                             .get();
      for (auto &&info : devicesInfo) {
        std::wstring_view nameView = info.Name();
        if (nameView.find(L"Daydream") != std::wstring_view::npos ||
            nameView.find(L"daydream") != std::wstring_view::npos) {
          StartBluetoothHandler(hwnd, 0, std::wstring(info.Id()));
          g_pairing = false;
          winrt::uninit_apartment();
          ScheduleMinimize(hwnd);
          return;
        }
      }
      ...
      g_advToken = g_advWatcher.Received(
          [hwnd](BluetoothLEAdvertisementWatcher const &,
            ...
            if (nameView.find(L"Daydream") != std::wstring_view::npos ||
              ...
              for (auto &&uuid : args.Advertisement().ServiceUuids()) {
                if (uuid ==
                        winrt::guid("0000fe55-0000-1000-8000-00805f9b34fb") ||
                    uuid ==
                        winrt::guid("0000fef5-0000-1000-8000-00805f9b34fb")) {
                  isDaydream = true;
                  break;
                }
              }
            ...
            if (isDaydream) {
              ...
              std::thread([hwnd, addr]() {
                try {
                  winrt::init_apartment();
                  auto device =
                      BluetoothLEDevice::FromBluetoothAddressAsync(addr).get();
                  if (device) {
                    if (device.DeviceInformation().Pairing().CanPair()) {

                      // Custom pairing to fix Code 19
                      auto custom =
                          device.DeviceInformation().Pairing().Custom();
                      std::shared_ptr<winrt::event_token> token =
                          std::make_shared<winrt::event_token>();
                      *token = custom.PairingRequested(
                      ...
                      UpdateStatus(hwnd, "Controller found, but Windows says "
                                         "it cannot be paired.");
                    }
                  }
                } catch (...) {
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
  ...
  switch (uMsg) {
  case WM_CREATE:
    CreateWindowA("BUTTON", "Install SteamVR Driver",
                  WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, 20, 20,
                  200, 30, hwnd, (HMENU)IDC_BTN_INSTALL,
                  (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
    CreateWindowA("BUTTON", "Connect Controller",
                  WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, 20, 60,
                  200, 30, hwnd, (HMENU)IDC_BTN_PAIR,
                  (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);

    CreateWindowA("BUTTON", "Hand (Needs SteamVR Restart):",
                  WS_VISIBLE | WS_CHILD | BS_GROUPBOX, 240, 10, 220, 80, hwnd,
                  NULL, (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE),
                  NULL);
    hRightRadio =
        CreateWindowA("BUTTON", "Right",
                      WS_VISIBLE | WS_CHILD | BS_AUTORADIOBUTTON | WS_GROUP,
                      250, 35, 100, 20, hwnd, (HMENU)IDC_RADIO_RIGHT,
                      (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
    hLeftRadio = CreateWindowA(
        "BUTTON", "Left", WS_VISIBLE | WS_CHILD | BS_AUTORADIOBUTTON, 250, 60,
        100, 20, hwnd, (HMENU)IDC_RADIO_LEFT,
        (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
    ...
    hStatusLabel =
        CreateWindowA("STATIC", "Ready.", WS_VISIBLE | WS_CHILD, 20, 110, 440,
                      20, hwnd, (HMENU)IDC_LABEL_STATUS,
                      (HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), NULL);
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  HWND hwnd = CreateWindowExA(
      0, CLASS_NAME, "Daydream Controller Manager",
      WS_OVERLAPPEDWINDOW & ~WS_THICKFRAME & ~WS_MAXIMIZEBOX, CW_USEDEFAULT,
      CW_USEDEFAULT, 500, 180, NULL, NULL, hInstance, NULL);
