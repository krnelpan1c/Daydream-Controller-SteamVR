...
# Usage
Download the latest release Driver.zip and extract it to a more permanent location on your PC (e.g. not your desktop) then run the executable in the bin\win64 folder. Click Install SteamVR driver, then start SteamVR. Press the Home button on your controller to pair the controller, and in Controller settings click the Assign button for the hand you want to assign, then press a button on the controller to assign it to that hand. Set your button mappings if the defaults don't suit you, then restart SteamVR and reopen the app. The app needs to run in the background but automatically minimizes to the system tray and will automatically close when SteamVR closes.
